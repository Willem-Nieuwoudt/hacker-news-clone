      ...
      {news.map(({ id, url, title, points, comments_count }) => (
        <div className='news-item' key={id}>
          <h2 className='news-item-title'>
            <a className='news-item-title-a' href={url}>
              {title}
            </a>
          </h2>

          <div className='news-item-details'>
            <span className='news-item-details-span'>{points || 0} point(s)</span>

            <Link href='/news/[id]' as={`/news/${id}`}>
              <a className='news-item-details-a'>{comments_count || 0} comment(s)</a>
            </Link>
          </div>
        </div>
      ))}
